...
def _call_memory_tool(self, tool_name: str, args: Dict) -> Dict:
    if tool_name == "store_memory":
        memory = self.memory_service.store_memory(
            ...
            tags=args.get("tags", []),  # ❌ ERROR: not supported
            metadata={}
        )
        return {"success": True, "memory_id": memory.memory_id}  # ❌ Wrong type
...
def _call_memory_tool(self, tool_name: str, args: Dict) -> Dict:
    if tool_name == "store_memory":
        ...
        memory = self.memory_service.store_memory(
            ...
            content=args["content"],
            memory_level=args.get("memory_level", "mid"),
            metadata=metadata  # ✅ FIXED: tags in metadata
        )
        return {"success": True, "memory_id": memory["memory_id"]}  # ✅ FIXED: dict access
```
...
await response.drain()  # ⚠️ DeprecationWarning
...
**Status**: ✅ Fixed in 3 locations (lines 195, 203, 207)
...
**Root Cause**:
- Database table `project_sessions` **缺少外键约束**
- SQLAlchemy模型定义了外键: `ForeignKey('code_projects.project_id')`
- 但数据库表中外键未创建(表是在模型更新前创建的)
...
**修复脚本**: `scripts/fix_foreign_keys.sql`
...
### Before Fixes
- ❌ `store_memory` tool fails with TypeError
- ❌ SQLAlchemy session rollback errors
- ⚠️ Deprecation warnings in logs
- ❌ Enterprise server unusable for production

### After Fixes
- ✅ `store_memory` tool works correctly
- ✅ No session rollback errors
- ✅ Clean logs with no warnings
- ✅ Enterprise server production-ready
...
**MCP v2.0.0 - All Bug Fixes Complete!** 🎉
